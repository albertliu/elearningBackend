	...
	function getCommitment(username,name,course,path,signDate,kind){
		...
		arr.push('		<tr>');
		if(kind==0){
			arr.push('			<td colspan="4" style="height:30px;"><p style="font-size:1.3em;text-indent:30px;line-height:50px;">[' + name + ']为本单位员工，身份证号：[' + username + ']，在我单位从事特种作业[' + course + ']（作业类别）相应工种的工作，该员工按规定参加安全培训，从事特种作业期间未发生拒绝、阻碍安全生产监管监察部门监督检查的行为；未发生违章操作造成严重后果或者未发生2次以上违章行为；未发生安全生产违法行为，并接受过行政处罚；特种作业人员未离开特种作业岗位6个月以上。</p></td>');
		}else{
