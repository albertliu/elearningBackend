		...
		$.post(uploadURL + "/public/postCommInfo", {proc:"getClassStudyOnline", params:{classID:nodeID, mark:keyID, theDate:$("#theDate").val()}}, function(data){
			...
			if(data>""){
				$.each(data,function(iNum,val){
					...
					arr.push("<td align='center' " + (val["attendance"]>=val["hours"] ? "style='background:#00FFFF;'" : "") + " title='完成课时:线上/线下'>" + val["attendance"] + ":" + val["attendanceOnline"] + "/" + val["attendanceOffline"] + "</td>");	//课时
					...
					arr.push("<td align='left' title='" + rs[val["result"]] + "'>" + nullNoDisp(val["score"]) + "&nbsp;/&nbsp;" + nullNoDisp(val["score2"]) + "</td>");
					arr.push("</tr>");
				});
			}
			arr.push("</tbody>");
