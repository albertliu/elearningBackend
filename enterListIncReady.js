	...
	$(document).ready(function (){
		if(currHost==""){	//公司用户只能看自己公司内容
			...
			getComList("searchEnterClassID","v_classInfo","classID","className","host='" + currHost + "' order by ID desc",1);
		...
		//$.getJSON("enterControl.asp?op=getEnterList",function(data){
			...
			if(ar>""){
				...
				$.each(ar,function(iNum,val){
					...
					arr.push("<td class='link1'><a href='javascript:showStudentExamStat(" + ar1[0] + ",\"" + ar1[2] + "\",0,0);'>" + nullNoDisp(ar1[15]) + "</a></td>");
					c = ar1[10];
					if(c>0){
						c = c;
					}else{
