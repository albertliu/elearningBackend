...
<script language="javascript">
	...
	$(document).ready(function (){
		...
		$('#avatar').change(function() {
			...
			$.ajax({
				url: uploadURL + "/outfiles/uploadSingle?upID=classArchive&username=" + $("#ID").val() + "&refID=A&keyID=&currUser=" + currUser + "&host=" + currHost,
				type: 'POST',
				data: formData,
				processData: false,
				contentType: false,
			...
			if(ar>""){
				...
				$.each(ar,function(iNum,val){
					...
					if(reexamine == 1){
						...
						arr.push("<td class='left' " + (ar1[28]==1 && bc>"" ? "style='background:" + bc + ";'" : "") + ">" + ar1[27] + "</td>");	// 换证日期
					}
					if(photo == 0){
						arr.push("<td class='left'>" + ar1[41] + "</td>");	//source
						arr.push("<td class='left' " + (Number(ar1[58])>=Number(ar1[57]) ? "style='background:#00FFFF;'" : "") + " title='应修课时" + ar1[57] + " 实修:线上/线下'>" + ar1[58] + ":" + (ar1[58]-ar1[59]).toFixed(2) + "/" + ar1[59] + "</td>");	//实修课时
						if(ar1[53]==''){
						...
						photo_size = ar1[36];
						if(photo_size > 99 || photo_size ==0 || (photo_type !== ".jpg" && photo_type !== ".jpeg")){	//根据照片类型或文件大小，显示不同背景颜色
						...
						if(ar1[42] > "" || ar1[43] > "" || ar1[44] > ""){	//工作证明、社保证明、居住证三选一, 
							...
							arr.push("<td class='center' title='" + jobtt[t] + " 大小:" + jobsize[t] + "k' style='background-color:" + jobbc[t] + "'><img id='" + jobid[t] + ar1[4] + "' src='users" + (ar1[42] || ar1[43] || ar1[44]) + "?times=" + (new Date().getTime()) + "' onclick='showCropperInfo(\"users" + (ar1[42] || ar1[43] || ar1[44]) + "\",\"" + ar1[4] + "\",\"" + jobid[t] + "\",\"\",0,1)' style='width:60px;background: #ccc;border:2px " + (jobsize[t] > 1000 ? "#FF0000" : "#fff") + " solid;box-shadow: 0 0 1px rgba(0, 0, 0, 0.8);-moz-box-shadow: 0 0 1px rgba(0, 0, 0, 0.8);-webkit-box-shadow: 0 0 1px rgba(0, 0, 0, 0.8);'></td>");
						}else{
							arr.push("<td class='center'>&nbsp;</td>");
						}
	...
	function generateEntryDoc(k){
		...
		if(confirm("确定要生成这" + selCount + "个" + f[k] + "吗？")){
			$.post(uploadURL + "/outfiles/generate_emergency_exam_materials_byclass?refID=" + nodeID + "&keyID=" + k + "&registerID=" + currUser + "&kindID=0&host=" + currHost, {selList:selList}, function(data){
				if(data>"0"){
