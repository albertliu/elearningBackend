...
if(op == "getApplyListByBatch"){
	...
	while (!rs.EOF){
		...
		//57
		result += "|" + rs("hours").value + "|" + rs("attendance").value + "|" + rs("attendanceOffline").value + "|" + rs("applyDetailCount").value;
		rs.MoveNext();
	}
	rs.Close();
	result = result.substr(2);
