...
if(op == "getDiplomaList"){
	...
	//如果有条件，按照条件查询
	if(where > ""){ // 有条件
		where = "(name like('%" + where + "%') or unit like('%" + where + "%') or username='" + where + "' or diplomaID='" + where + "')";
	}
	//如果有公司
	if(host > ""){ // 
		s = "host='" + host + "'";
...
if(op == "getApplyListByBatch"){
	...
	ssql = "SELECT kindName,qty,hostName,memo,regDate,registerName" + sql + " order by ID";
	sql = "SELECT *, [dbo].[getStudentMaterialSize](username,5) as employe_size, [dbo].[getStudentMaterialSize](username,8) as social_size, [dbo].[getStudentMaterialSize](username,6) as job_size, dbo.getEnterAttendance(enterID) as attendance, dbo.getEnterAttendanceOffline(enterID) as attendanceOffline,[dbo].[getApplyDetailCount1](ID) as applyDetailCount, [dbo].[getApplyDetailPayment](ID) as applyDetailPayment" + sql + " order by passNo, ID";
	...
	while (!rs.EOF){
		...
		//57
		result += "|" + rs("hours").value + "|" + rs("attendance").value + "|" + rs("attendanceOffline").value + "|" + rs("applyDetailCount").value + "|" + rs("applyDetailPayment").value;
		rs.MoveNext();
	}
	rs.Close();
	result = result.substr(2);
