   	...
   	/*	
		$("#formLoad").ajaxForm(function(data){
			...
			if(data==""){
				...
				$.get("readExcel.asp?kindID=getK3payList&fName=" + escape(data) + "&times=" + (new Date().getTime()),function(re){
					alert(unescape(re));
					var ar = new Array();
		...
		$("#restore").click(function(){
	    	...
	    	if(s>""){
	    		window.clearInterval(timer);
					$.get("readExcel.asp?kindID=getK3payList&fName=" + escape(s) + "&times=" + (new Date().getTime()),function(re){
						//alert(unescape(re));
						var ar = new Array();
						ar = unescape(re).split("|");
						if(ar[0]==-1){
