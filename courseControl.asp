...
if(op == "getNodeInfo"){
	...
	if (!rs.EOF){
		...
		//20
		result += "|" + rs("price").value + "|" + rs("reexamine").value + "|" + rs("reexamineName").value + "|" + rs("shortName").value + "|" + rs("price1").value + "|" + rs("priceExam").value;
	}
	rs.Close();
	Response.Write(escape(result));
}
...
if(op == "update"){
	result = 0;
	if(result == 0){
		sql = "exec updateCourseInfo " + nodeID + ",'" + String(Request.QueryString("courseID")) + "','" + unescape(String(Request.QueryString("courseName"))) + "','" + unescape(String(Request.QueryString("shortName"))) + "','" + String(Request.QueryString("hours")) + "','" + String(Request.QueryString("completionPass")) + "','" + String(Request.QueryString("deadline")) + "','" + String(Request.QueryString("deadday")) + "','" + String(Request.QueryString("period")) + "'," + kindID + "," + String(Request.QueryString("reexamine")) + "," + status + "," + String(Request.QueryString("mark")) + ",'" + refID + "','" + String(Request.QueryString("price")) + "','" + String(Request.QueryString("price1")) + "','" + String(Request.QueryString("priceExam")) + "','" + host + "','" + memo + "','" + currUser + "'";
