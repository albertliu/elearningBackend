	...
	function outputExcelBySQL(id,key,p1,p2,p3){
		...
		if(id=='x05'){
			...
			floatTitle = p2;
			floatItem = p3;
			sql = "select SNo,name,sexName,educationName,username,mobile,unit,dept1Name,IDdateStart,IDdateEnd,diplomaID,score1,score2,statusName from v_applyInfo where refID=" + p1 + " order by passNo,ID";
		}
		if(id=='x06'){
...
　　　		}
	...
	//nodeID: ID; op: 0 浏览 1 新增  2 编辑  3 删除  4 审批; mark: 0 不动作  1 有修改时刷新列表  2 有修改时刷新对象
	function showSignatureInfo(nodeID,refID,op,mark,after){
		asyncbox.open({
			id: "wsignature",
...
　　　		}
	...
	//nodeID: ID; op: 0 浏览 1 新增  2 编辑  3 删除  4 审批; mark: 0 不动作  1 有修改时刷新列表  2 有修改时刷新对象
	function showExamPlaceInfo(nodeID,op,mark){
		asyncbox.open({
			id: "examPlaceInfo",
			url:"examPlaceInfo.asp?nodeID=" + nodeID + "&op=" + op + "&p=1&times=" + (new Date().getTime()),
			title: "考点空位信息",
			width: 580,
			height: 580,
			cover : {
