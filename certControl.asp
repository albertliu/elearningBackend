...
if(op == "getCertList"){
	...
	while (!rs.EOF){
		...
		//8
		result += "|" + rs("agencyID").value + "|" + rs("agencyName").value + "|" + rs("memo").value + "|" + rs("regDate").value + "|" + rs("registerID").value + "|" + rs("registerName").value + "|" + rs("hours").value + "|" + rs("host").value + "|" + rs("hostName").value + "|" + rs("termExt").value;
		rs.MoveNext();
	}
/**/
	Session(op) = ssql;
...
if(op == "update"){
	result = 0;
	if(result == 0){
		sql = "exec updateCertificateInfo " + nodeID + ",'" + String(Request.QueryString("certID")) + "','" + unescape(String(Request.QueryString("certName"))) + "','" + String(Request.QueryString("term")) + "','" + String(Request.QueryString("termExt")) + "'," + String(Request.QueryString("agencyID")) + "," + kindID + "," + status + ",'" + host + "','" + memo + "','" + currUser + "'";
