...
<script language="javascript">
	...
	function getNodeInfo(id){
		$.get("agencyControl.asp?op=getNodeInfo&nodeID=" + id + "&times=" + (new Date().getTime()),function(re){
			...
			if(ar > ""){
				...
				$("#upload1").html("<a href='javascript:showLoadFile(\"agency_logo\",\"" + ar[1] + "\",\"agency\",\"\");' style='padding:3px;'>上传</a>");
				setButton();
			}else{
				jAlert("该信息未找到！","信息提示");
				setEmpty();
	...
	function saveNode(){
		...
		$.get("agencyControl.asp?op=update&nodeID=" + $("#ID").val() + "&refID=" + $("#agencyID").val() + "&agencyName=" + escape($("#agencyName").val()) + "&title=" + escape($("#title").val()) + "&linker=" +  escape($("#linker").val()) + "&kindID=" + $("#kindID").val() + "&status=" + $("#status").val() + "&phone=" +  escape($("#phone").val()) + "&email=" + escape($("#email").val()) + "&address=" + escape($("#address").val()) + "&memo=" + escape($("#memo").val()) + "&times=" + (new Date().getTime()),function(re){
			//alert(unescape(re));
			var ar = new Array();
			ar = unescape(re).split("|");
			if(ar[0] == 0){
	...
	<div style="width:100%;float:left;margin:0;">
		<div style="border:solid 1px #e0e0e0;width:99%;margin:1px;background:#ffffff;line-height:18px;">
			...
			<tr>
				...
				<td><input type="text" id="agencyName" size="25" /></td>
