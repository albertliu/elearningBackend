   	...
   	/*	
		$("#formLoad").ajaxForm(function(data){
			...
			if(data==""){
				...
				$.get("readExcel.asp?kindID=getK3employeeList&fName=" + escape(data) + "&times=" + (new Date().getTime()),function(re){
					alert(unescape(re));
					var ar = new Array();
		...
		$("#restore").click(function(){
	    	...
	    	if(s>""){
	    		...
	    		//alert("C"+refID);
					$.get("readExcel.asp?kindID=getK3employeeList&fName=" + escape(s) + "&refID=" + refID + "&times=" + (new Date().getTime()),function(re){
						//jAlert(unescape(re));
						var ar = new Array();
						ar = unescape(re).split("|");
						if(ar[0]==-1){
							...
							if(ar[1]>0 && ar[0]>-1){
								...
								s = "导入的数据中包括<font color='red'>" + ar[2] + "</font>条记录需要核实，这部分数据未导入。\n<hr size='1' noshadow>" + ar[5];
								jAlert(s,"错误提示");
							}
						}
					});
