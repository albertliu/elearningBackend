...
if(op == "getStudentExamList"){
	...
	sql = "select 0 as a, enterID, certName, startDate as examDate, cast(score as varchar) as score, statusName as resultName, diplomaID,batchID as ID from v_passcardInfo" + where;
	...
	rs = conn.Execute(sql);
	while (!rs.EOF){
		result += "%%" + rs("enterID").value + "|" + rs("certName").value + "|" + rs("examDate").value + "||" + rs("score").value + "|" + rs("resultName").value + "|" + rs("diplomaID").value + "|" + rs("a").value + "|" + rs("ID").value;
		rs.MoveNext();
	}
	rs.Close();
	result = result.substr(2);/**/
...
if(op == "doEnter"){
	...
	sql = "exec doEnter '" + nodeID + "','" + String(Request.QueryString("classID")) + "','" + String(Request.QueryString("price")) + "','" + String(Request.QueryString("invoice")) + "','" + String(Request.QueryString("projectID")) + "','" + item + "'," + kindID + "," + String(Request.QueryString("type")) + "," + status + ",'" + String(Request.QueryString("datePay")) + "','" + String(Request.QueryString("dateInvoice")) + "','" + String(Request.QueryString("dateInvoicePick")) + "','" + String(Request.QueryString("currDiplomaID")) + "','" + String(Request.QueryString("currDiplomaDate")) + "','" + String(Request.QueryString("fromID"))  + "',0,'" + memo + "','" + currUser + "'";
