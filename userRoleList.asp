...
<script language="javascript">
	...
	function getRoleList(){
		$.get("userControl.asp?op=getRoleList&userName=" + refID + "&times=" + (new Date().getTime()),function(data){
			var ar = new Array();
			...
			arr.push("<th width='40%'>权限</th>");
			arr.push("<th width='40%'>说明</th>");
	...
	function getUserRoleList(){
		$.get("userControl.asp?op=getRoleListByUser&userName=" + refID + "&times=" + (new Date().getTime()),function(data){
			var ar = new Array();
			ar = (unescape(data)).split("%%");
			//alert(ar);
			$("#taskUserCover").empty();
	...
	function doAddUser(user){
		$.get("userControl.asp?op=addRole2User&userName=" + refID + "&uID=" + user + "&times=" + (new Date().getTime()),function(re){
			if(re>""){
				getUserRoleList();
				getRoleList();
			}
	...
	function doRemoveUser(nodeID){
		jConfirm("确实要退出该角色吗？","确认对话框",function(r){
			if(r){
				$.get("userControl.asp?op=removeRole4User&uID=" + nodeID + "&times=" + (new Date().getTime()),function(re){
					if(re>""){
						getUserRoleList();
						getRoleList();
					}
...
<div id='layout' style="width:99%;float:left;margin:0;">	
	...
	<div style="width:48%;float:left;margin:2;">
		<div style="color:orange;margin:0;padding:5px;text-align:center;">未分配角色</div>
	...
	<div style="width:48%;float:right;margin:2;">
		<div style="color:orange;margin:0;padding:5px;text-align:center;">已分配角色</div>
		<div style="border:solid 1px #e0e0e0;width:100%;margin:1px;background:#ffffff;line-height:18px;">
